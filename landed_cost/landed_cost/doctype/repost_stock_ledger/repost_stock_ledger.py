...
class RepostStockLedger(Document):
	...
	def on_submit(self):
		if len(self.vouchers) > 5:
			job_name = "repost_stock_" + self.name
			frappe.enqueue(
				method="landed_cost.landed_cost.doctype.repost_stock_ledger.repost_stock_ledger.start_repost",
				stock_repost_doc=self.name,
				is_async=True,
				job_name=job_name,
			)
...
@frappe.whitelist()
def start_repost(stock_repost_doc=str) -> None:
	frappe.flags.through_repost_accounting_ledger = True
	frappe.flags.through_repost_stock_ledger = True
	if stock_repost_doc:
		repost_doc = frappe.get_doc("Repost Stock Ledger", stock_repost_doc)
		...
		if repost_doc.docstatus == 1:
			...
			for x in repost_doc.vouchers:
				...
				if doc.doctype in ["Stock Entry"]:
					if not repost_doc.delete_cancelled_entries:
						doc.docstatus = 2
						doc.update_stock_ledger()
						doc.make_gl_entries_on_cancel()
						doc.repost_future_sle_and_gle()
						# doc.update_transferred_qty()
						# doc.delete_linked_stock_entry()

					doc.docstatus = 1
					doc.calculate_rate_and_amount_on_repost()
					doc.update_stock_ledger()
					doc.make_gl_entries()
					doc.repost_future_sle_and_gle()
			...
			_("{0} {1} not allowed to be reposted. Modify {2} to enable reposting.").format(
				...
				frappe.bold(
					frappe.utils.get_link_to_form(
						"Repost Stock Ledger Settings", "Repost Stock Ledger Settings"
					)
				),
			)
		)
